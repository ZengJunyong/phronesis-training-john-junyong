...
const { RebillyAPI } = require("rebilly-js-sdk");
...
router.post("/invoices", async (req, res) => {
  const invoices = await api.invoices.getAll({
    filter: `customerId:cus_01J63Y0C1XZ5Q3M4G9W246KX2P`,
  });
  res.json(invoices);
});
...
router.post("/deposit-with-strategy", async (req, res) => {
  ...
  try {
    const {
      fields: { token: exchangeToken },
    } = await api.customerAuthentication.login({
      data: {
        mode: "passwordless",
        customerId,
      },
    ...
    } = await api.depositRequests.create({
      data: {
        websiteId: REBILLY_WEBSITE_ID,
        customerId,
        currency,
